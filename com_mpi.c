...
#define DEFAULT_LIST_LENGTH 256
#define COLLECTIVE_RANK -1
#define NUM_OPS 13
...
void writeMetadataToFile(vertex, int, int, int);
...
int MPI_Init_thread( int *argc, char ***argv, int required, int *provided ) {
 ...
 // _EXTERN_C_ int MPI_Init(int *argc, char ***argv) { 
   ...
   //printf("In Init_thread.\n");
    ...
    // FILE *fi;
    // fi = fopen("init.test", "w+");
    // fprintf(fi, "Init worked.\n");
    // fclose(fi);
  
  vertex newNode;
  newNode.startTime = 0;
  strcpy(newNode.label, "MPI_Init");
  
  
  operationCounter = collectiveCounter = 0;
  ...
  
 {
  ...
  sprintf(filename, "meta%d.m", myRank);
  
  //write to file
  
  // fp = fopen(filename, "a");
  // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, -1);
  // fclose(fp);
  newNode.endTime = MPI_Wtime();
  writeMetadataToFile(newNode, COLLECTIVE_RANK, -1, -1);
...
_EXTERN_C_ int MPI_Finalize() { 
 ...
 FILE *fp = fopen("final.test","w");
 fprintf(fp, "Finalize worked.\n");
 fclose(fp);
  
 vertex newNode;
 newNode.startTime = MPI_Wtime();
 strcpy(newNode.label, "MPI_Finalize");
 newNode.endTime = 0;
 
 writeMetadataToFile(newNode, COLLECTIVE_RANK, -1, -1);
 {
  _wrap_py_return_val = PMPI_Barrier(MPI_COMM_WORLD);
  PMPI_Reduce(&operationCounter, &totalOperationCount, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
...
_EXTERN_C_ int MPI_Barrier(MPI_Comm comm) { 
 ...
 double operationStartTime = MPI_Wtime();

 {
  _wrap_py_return_val = PMPI_Barrier(comm);
}

 double operationEndTime = MPI_Wtime();
 
 strcpy(newNode.label, "MPI_Barrier");
  
 //write
 collectiveCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, collectiveCounter);
 // fclose(fp);
  
 newNode.endTime = MPI_Wtime();
 
 writeMetadataToFile(newNode, COLLECTIVE_RANK, collectiveCounter, -1);
 //write op + metadata to file
...
_EXTERN_C_ int MPI_Alltoall(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm) { 
 ...
 strcpy(newNode.label, "MPI_Alltoall");
 //write
 collectiveCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, collectiveCounter);
...
_EXTERN_C_ int MPI_Scatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm) { 
 ...
 strcpy(newNode.label, "MPI_Scatter");
 //write
 collectiveCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, collectiveCounter);
...
_EXTERN_C_ int MPI_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm) { 
 ...
 strcpy(newNode.label, "MPI_Gather");
 //write
 collectiveCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, collectiveCounter);
...
_EXTERN_C_ int MPI_Reduce(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm) { 
 ...
 strcpy(newNode.label, "MPI_Reduce");
 //write
 collectiveCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, collectiveCounter);
...
_EXTERN_C_ int MPI_Allreduce(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, MPI_Comm comm) { 
 ...
 strcpy(newNode.label, "MPI_Allreduce");
 //write
 collectiveCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, COLLECTIVE_RANK, collectiveCounter);
...
//int PMPI_Send(const void *, int, MPI_Datatype={int}, int, int, MPI_Comm={int})
_EXTERN_C_ int PMPI_Send(const void *buf, int count, int datatype, int dest, int tag, int comm);
_EXTERN_C_ int MPI_Send(const void *buf, int count, int datatype, int dest, int tag, int comm) { 
    int _wrap_py_return_val = 0;

    FILE *fsend = fopen("_send.test","a");
    fprintf(fsend, "Send worked.\n");
    fclose(fsend);
  ...
  double operationStartTime = MPI_Wtime();
 {
  ...
  strcpy(newNode.label, "MPI_Send");
  newNode.senderRank = myRank;
  newNode.receiverRank = dest;
  newNode.tag = tag;
  MPI_Type_size(datatype, &datatypeSize);
  newNode.messageSize = datatypeSize * count;
  //write
  operationCounter++;
  // fp = fopen(filename, "a");
  // fprintf(fp, "%s %d %d\n", __func__, myRank, operationCounter);
  // fclose(fp);
  newNode.endTime = MPI_Wtime();
  
  writeMetadataToFile(newNode, myRank, operationCounter, 1);
  //write op + metadata to file
    return _wrap_py_return_val;
}
...
_EXTERN_C_ int MPI_Isend(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request) { 
 ...
 strcpy(newNode.label, "MPI_Isend");
 newNode.senderRank = myRank;
 newNode.receiverRank = dest;
 newNode.tag = tag;
 MPI_Type_size(datatype, &datatypeSize);
...
_EXTERN_C_ int MPI_Recv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Status *status) { 
 ...
 strcpy(newNode.label, "MPI_Recv");
 newNode.senderRank = source;
 newNode.receiverRank = myRank;
 newNode.tag = tag;
 //write
 operationCounter++;
 // fp = fopen(filename, "a");
 ...
 writeMetadataToFile(newNode, myRank, operationCounter, 2);
 //write op + metadata to file
    return _wrap_py_return_val;
}
...
_EXTERN_C_ int MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request) { 
 ...
 strcpy(newNode.label, "MPI_Irecv");
 newNode.senderRank = source;
 newNode.receiverRank = myRank;
 newNode.tag = tag;
 //write
...
_EXTERN_C_ int MPI_Wait(MPI_Request *request, MPI_Status *status) { 
 ...
 strcpy(newNode.label, "MPI_Wait");
 newNode.senderRank = status->MPI_SOURCE;
 newNode.receiverRank = myRank;
 newNode.tag = status->MPI_TAG;
 //write
 operationCounter++;
 // fp = fopen(filename, "a");
 // fprintf(fp, "%s %d %d\n", __func__, myRank, operationCounter);
...
void writeMetadataToFile(vertex newNode, int myRank, int counter, int operationType) { //opType=1 for send, 2 for recv/wait, or 3
  char path[24];
  char dir[8];
  strcpy(dir, "trace/");
  ...
  //fp = fopen(filename, "a");
  fprintf(fp, "%s %d %d %f %f", newNode.label, myRank, counter, newNode.startTime, newNode.endTime);
  if (operationType == 1) {
    fprintf(fp, " %d %d %d %d", newNode.senderRank, newNode.receiverRank, newNode.tag, newNode.messageSize);
  }
  else if (operationType == 2) {
    fprintf(fp, " %d %d %d", newNode.senderRank, newNode.receiverRank, newNode.tag);
  }
  fprintf(fp, "\n");  
  fclose(fp);
}
...
{
  int i, flag, *tag_ub, iprov;
  MPI_Comm comm;
  MPI_Errhandler errhandler;

/*  flag = MPI_Init(argc, argv); */
  flag = MPI_Init_thread( argc, argv, MPI_THREAD_SINGLE, &iprov );
  comm = MPI_COMM_WORLD;
